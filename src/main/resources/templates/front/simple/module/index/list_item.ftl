...
<#--一个列表项-->
    <div class="list-item px-1 py-4 mt-xl-0 mt-3 border-bottom">
        <#--文章标题-->
        <a href="/archives/${post.postUrl!}" class="text-body">
            <h4 class="post-title font-weight-bold text-break">
                ${post.postTitle!}
            </h4>
        </a>
        <#--文章摘要-->
        <span class="post-summary mt-4 d-block text-muted text-break">${post.postSummary!}</span>
                ...
                <span class="mr-3 font-weight-bold d-md-inline d-none">
                    ...
                    <#list post.tags as tag>
                        <a class="text-body mr-1"
                           href="/tags/${tag.tagUrl!}">${tag.tagName!}</a>
                    </#list>
                </span>
            </#if>
            <#--日期-->
            <span class="mr-3">
                <i class="fas fa-calendar-alt mr-2"></i>${post.postDate?string["yyyy年MM月dd日"]!}
            </span>
            <#--多少次浏览-->
            <#--<span>-->
