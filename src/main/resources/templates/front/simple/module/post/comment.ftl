...
<section id="comment" class="mb-5">
    <link type="text/css" rel="stylesheet" href="/static/loaders/loaders.css">
    <style>
        * {
            ...
            <#if comments.totalPages gt 1>
                <ol class="page-nav">
                    <#if comments.hasPrevious()>
                        <li>
                            <a href="?page=${comments.number-1}#comments-list" title="上一页">←</a>
                        </li>
                    </#if>
                    <li>
                        <#list 1..comments.totalPages as r>
                            <#if r == comments.number+1>
                                <a href="?page=${r-1}#comments-list" style="color: red;">${r}</a>
                            <#else>
                                <a href="?page=${r-1}#comments-list">${r}</a>
                            </#if>
                        </#list>
                    </li>
                    <#if comments.hasNext()>
                        <li>
                            <a href="?page=${comments.number+1}#comments-list" title="下一页">→</a>
                        </li>
                    </#if>
                </ol>
            </#if>
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data==true){
                        ...
                        $(".native-message").html("<span>"+"成功"+"</span>");
                        $(".native-message").fadeIn(1000);
                    }else{$(".native-message").html("<span>"+"失败"+"</span>");
                        $(".native-message").fadeIn(1000);
                    }
                    setTimeout(function () {
